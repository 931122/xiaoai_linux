...
static int format_ms_to_alsa(const PCMFORMATEX * wavfmt,
...
static int set_hwparams(struct recorder * rec,  const PCMFORMATEX *wavfmt,
...
static int set_params(struct recorder *rec, PCMFORMATEX *fmt,
        unsigned int buffertime, unsigned int periodtime)
{
    int err;
    PCMFORMATEX defmt = DEFAULT_FORMAT;
...
#else
    char *buf[2];/* channel num */
    buf[0] = (char *)malloc(4096);
    buf[1] = (char *)malloc(4096);

    data = rec->audiobuf;
    while (count > 0) {
        r = snd_pcm_readn(handle, buf, count);
        if (r == -EAGAIN || (r >= 0 && (size_t)r < count)) {
            snd_pcm_wait(handle, 100);
        } else if (r < 0) {
            if(xrun_recovery(handle, r) < 0) {
        ...
        if (r > 0) {
            /* get channle 2 record data */
            memcpy(data, buf[1], r * rec->bits_per_frame / 8);
...
static void * record_thread_cb(void * para)
{
...
{
    int err;
    err = pthread_create(tidp, NULL, record_thread_cb, (void *)para);
    if (err != 0)
        return err;
...
static int prepare_rec_buffer(struct recorder * rec )
{
    /* the read and callback Write is blocked, currently only support one buffer,
     * if overrun too much, need more buffer and another new thread
     * to write the audio to network */
...
static int open_recorder_internal(struct recorder * rec,
        record_dev_id dev, PCMFORMATEX * fmt)
{
    int err = 0;
...
static int get_pcm_device_cnt(snd_pcm_stream_t stream)
{
    ...
    while (*n != NULL) {
        ...
        dbg("get_pcm_device_cnt io[%s]name[%s]vs filter[%s]\n", io, name, filter);
...
int open_recorder(struct recorder * rec, record_dev_id dev, PCMFORMATEX * fmt)
{
    int ret = 0;
    if(!rec )
